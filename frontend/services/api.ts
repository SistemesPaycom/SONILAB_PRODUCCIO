...
async listProjects() {
  ...
  // Folders
  async createFolder(name: string, parentId: string | null, category?: 'files' | 'media') {
    return request<any>(`/folders`, { method: 'POST', body: { name, parentId, category } });
  },
  ...
  // Projects / Jobs
  async createProject(payload: { name: string; mediaDocumentId: string; settings?: any; parentFolderId?: string }) {
