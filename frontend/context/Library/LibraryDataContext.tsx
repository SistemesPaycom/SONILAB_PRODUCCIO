...
interface LibraryDataState {
  ...
  currentMediaFolderId: string | null;
...
export type LibraryDataAction =
  ...
  | { type: 'CREATE_FOLDER'; payload: { name: string; parentId: string | null; category?: 'files' | 'media' } }
  ...
  | { type: 'SET_CURRENT_MEDIA_FOLDER'; payload: string | null }
...
const ACTIVE_MEDIA_FOLDER_KEY = 'snlbpro_active_media_folder';
  ...
  switch (action.type) {
    ...
    case 'CREATE_FOLDER': {
      ...
      const newFolder: Folder = {
        ...
        isDeleted: false,
        type: 'folder',
        category: action.payload.category ?? 'files',
      };
    ...
    case 'SET_CURRENT_MEDIA_FOLDER':
      return { ...state, currentMediaFolderId: action.payload, selectedIds: new Set() };

    case 'CLEAR_SELECTION':
      return { ...state, selectedIds: new Set() };

    case 'SET_VIEW': {
      const isTabSwitch = action.payload === 'library' && state.view === 'library';
      return {
        ...state,
        view: action.payload,
        currentFolderId: isTabSwitch ? state.currentFolderId : null,
        currentMediaFolderId: isTabSwitch ? state.currentMediaFolderId : null,
        selectedIds: new Set(),
      };
    }
...
interface LibraryDataContextValue {
  ...
  useBackend: boolean;
  reloadTree: (opts?: { view?: ViewType; folderId?: string | null; mediaFolderId?: string | null }) => Promise<void>;
  createFolderRemote: (name: string, parentId: string | null, category?: 'files' | 'media') => Promise<void>;
...
export const LibraryDataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const reloadTree = async (opts?: { view?: ViewType; folderId?: string | null; mediaFolderId?: string | null }) => {
    const previousFolderId = opts?.folderId !== undefined ? opts.folderId : state.currentFolderId;
    const previousMediaFolderId = opts?.mediaFolderId !== undefined ? opts.mediaFolderId : state.currentMediaFolderId;
    ...
    const mediaFolderStillExists =
      previousMediaFolderId !== null &&
      normalizedFolders.some(f => f.id === previousMediaFolderId && !f.isDeleted);
    dispatch({
      ...
      payload: {
        ...
        currentMediaFolderId: mediaFolderStillExists ? previousMediaFolderId : null,
        view: targetView,
      },
    });
  };
  ...
  useEffect(() => {
    ...
    const storedMediaFolderId = restoredView !== 'trash' ? localStorage.getItem(ACTIVE_MEDIA_FOLDER_KEY) : null;
    if (!useBackend) {
      try {
        ...
        if (stored) {
          ...
          const mediaFolderStillExists = storedMediaFolderId &&
            parsed.folders?.some((f: any) => f.id === storedMediaFolderId && !f.isDeleted);
          dispatch({ type: 'SET_INITIAL_STATE', payload: {
            ...
            currentMediaFolderId: mediaFolderStillExists ? storedMediaFolderId : null,
          }});
        } else {
    ...
    reloadTree({ view: restoredView, folderId: storedFolderId || null, mediaFolderId: storedMediaFolderId || null })
      .catch(() => {
        dispatch({ type: 'SET_INITIAL_STATE', payload: { ...initialState } });
      });
  ...
  // Persistir la carpeta Media activa
  useEffect(() => {
    if (state.isLoading || state.view === 'trash') return;
    if (state.currentMediaFolderId) {
      localStorage.setItem(ACTIVE_MEDIA_FOLDER_KEY, state.currentMediaFolderId);
    } else {
      localStorage.removeItem(ACTIVE_MEDIA_FOLDER_KEY);
    }
  }, [state.currentMediaFolderId, state.view, state.isLoading]);
  ...
  const createFolderRemote = async (name: string, parentId: string | null, category?: 'files' | 'media') => {
    await api.createFolder(name, parentId, category);
    await reloadTree();
  };
