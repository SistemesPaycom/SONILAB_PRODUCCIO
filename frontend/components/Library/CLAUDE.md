...
## 10. Protección de extensiones en rename

En cualquier flujo de rename de esta carpeta, la extensión es NO editable por el usuario.

Patrón obligatorio:
1. Al abrir el input → `splitFileNameExt(name)` → `renameValue = base`, `renameExt = ext`
2. Input editable solo muestra `renameValue` (la base)
3. Badge no editable muestra `renameExt` a la derecha del input
4. Al confirmar → `newName = renameValue.trim() + renameExt`

Este patrón aplica a:
- Modal de rename (`renameModalOpen`)
- Modal de conflicto de mover — paso "Canviar nom" (`moveConflict.step === 'rename'`)
- Cualquier inline rename que se añada en el futuro en `LibraryFileItem.tsx`

Ver `splitFileNameExt` (helper a nivel de módulo en `SonilabLibraryView.tsx`) y la sección 7b de `SONILAB_PRODUCCIO/CLAUDE.md`.
