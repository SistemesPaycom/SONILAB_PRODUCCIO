...
interface FileItemProps {
  ...
  provenance?: 'Media' | 'Files';
  onMoveItemsToBackend?: (ids: string[], destinationFolderId: string | null) => Promise<void>;
...
export const FileItem: React.FC<FileItemProps> = ({
  ...
  const handleDrop = useCallback((draggedItemIds: string[], destinationFolderId: string | null, dropAction?: string) => {
      ...
      // Guard: media canònica no es pot moure com un fitxer clàssic fora del tab Media.
      // Dins del tab Media sí es permet (canvi de parentId, no còpia binària).
      ...
      if (hasCanonicalMediaInDrag && page !== 'media') return;
      ...
      onMoveItemsToBackend?.(draggedItemIds, destinationFolderId);
    },
    [dispatch, folders, currentFolderId, item.type, documents, page, onMoveItemsToBackend]
  );
        ...
        if (isDragAction && draggedElement) {
          ...
          if (dropTargetElement) {
            ...
            if (!dropAction) {
                ...
                // excepte dins del tab Media on sí es permet reorganitzar assets.
                const dragHasCanonicalMedia = draggedIds.some(id => {
                  const doc = documents.find(d => d.id === id);
                  return !!(doc && (doc as any).media && !(doc as any).refTargetId);
                });
                isCompatible = !dragHasCanonicalMedia || page === 'media';
      ...
      const onMouseUp = (upEvent: MouseEvent) => {
        ...
        try {
            ...
            if (isDragAction) {
              ...
              if (dropTargetElement) {
                  const dropAction = dropTargetElement.getAttribute('data-drop-action');
                  const rawDropId = dropTargetElement.getAttribute('data-id');
                  // '__root__' és el sentinel per a la carpeta arrel (id null) de les migas de pan
                  const finalDropTargetId: string | null = rawDropId === '__root__' ? null : rawDropId;
                  const draggedIds = selectedIds.has(item.id) ? Array.from(selectedIds) : [item.id];
                  ...
                  if (!dropAction) {
                    ...
                    canProceed = finalDropTargetId !== item.id && (!dropHasCanonicalMedia || page === 'media');
                  } else if (draggedIds.length === 1) {
                      const doc = documents.find(d => d.id === draggedIds[0]);
                      if (doc) {
                          const sType = doc.sourceType?.toLowerCase() || '';
              ...
              if (item.type === 'folder') { onEnterFolder?.(item.id); return; }
  ...
  }, [item.id, item.type, selectedIds, folders, documents, handleDrop, setIsDragging, setDropTargetId, dropTargetId, view, dispatch, onPreviewDocument, page]);
  ...
  const formatLabel = item.type === 'folder' ? (isProject ? 'Projecte' : isMediaFolder ? 'Media' : 'Carpeta') : isRef ? `LNK (${displayFormat})` : displayFormat;
    ...
    >
      ...
      >
        ...
        <span className="relative flex-shrink-0">
          {item.type === 'folder' ? (isProject ? '🗃️' : isMediaFolder ? '🗂️' : '📁') : getFileIcon(sourceType || '', isLocked)}
          {isRef && (
            <span
      ...
      {provenance !== undefined && (
        <div className="hidden sm:flex items-center px-4 min-w-0 overflow-hidden select-none whitespace-nowrap"
          style={{ color: 'var(--us-home-datetime-color)', fontSize: 'var(--us-home-datetime-size)' }}
        >
      ...
      <div className="z-10 flex items-center justify-center px-2 relative">
        {(() => {
          ...
          // Regles per tab:
          // Files (library): Copiar + Retallar sempre (carpetes, docs, LNK — excepte media canònica)
          // Media: solo Retallar
          // Projectes: cap de les dues
          const isFilesPage = page === 'library';
          const isMediaPage = page === 'media';
          const menuItems: { label: string; icon: string; action: () => void }[] = [];
          if (onRename && ids.length === 1) menuItems.push({ label: 'Renombrar', icon: '✏️', action: () => onRename(ids[0]) });
          if (onCopy && isFilesPage && !selectionHasCanonicalMedia) menuItems.push({ label: 'Copiar', icon: '📋', action: () => onCopy(ids) });
          // Files: bloca media canònica. Media: permet retallar qualsevol asset.
          const canCutHere = isFilesPage ? !selectionHasCanonicalMedia : isMediaPage;
          if (onCut && canCutHere) menuItems.push({ label: 'Retallar', icon: '✂️', action: () => onCut(ids) });
