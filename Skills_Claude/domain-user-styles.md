...
- **El Factory Reset (Settings → General → "Restablir configuració de fàbrica") preserva explícitamente los presets del usuario**. La clave `snlbpro_user_styles_<userId>` y `me.preferences.userStyles` en el backend NO se tocan por el reset, porque son trabajo creativo del usuario (pueden ser horas de afinado de tipografía/colores). Ver `docs/superpowers/specs/2026-04-07-reset-configuracio-frontend.md` (sección 3) para la lista completa de lo preservado vs lo borrado.
...
- **🔴 ANTI-PARPADEIG (regla crítica)**: NO uses **callback refs**, `setProperty(prop, value, 'important')`, `MutationObserver`, `useImportantStyleRef` ni añadas `!important` nuevo para aplicar las CSS vars `--us-*`. Estos patrones causaron el bug del parpadeig de la commit `ef60b36` (decenas de miles de mutaciones de `style` por segundo, fonts cambiando de tamaño constantemente). La forma correcta de consumir las vars del preset es **siempre** vía inline `style={{ fontFamily: 'var(--us-...)', fontSize: 'var(--us-...-size)', color: 'var(--us-...-color)', ... }}`. Si una clase Tailwind del tema interfiere (`text-white`, `text-gray-200`, `text-sm`, etc.), **ELIMINA la clase Tailwind**, no añadas `!important`. Las reglas del `ThemeContext` para `text-white`/`text-gray-*` NO tienen `!important`, así que un inline `style` gana por especificidad CSS estándar (verificado en `ThemeContext.tsx:181`).
- **Iconos SVG que escalan con el preset**: cuando un icono SVG vive dentro de un botón cuyo `font-size` se controla con `var(--us-...-size)`, el icono debe usar `className="w-[1em] h-[1em]"` (NO `w-4 h-4` ni mida fixa). Esto hace que el icono escale uniformemente con el texto. Aplica a `Icons.Folder/Pin/Film/Trash/FolderPlus/Upload` dentro de los botones de `SonilabLibraryView`. Para que el color del icono herede del preset, los SVG deben usar `stroke="currentColor"` (todos los de `icons.tsx` ya lo hacen).
- **Truncate en celdas flex/grid (header de tabla)**: cuando una celda con texto está dentro de un contenedor flex o grid, el `truncate` Tailwind necesita: (a) `min-w-0 overflow-hidden` en el wrapper externo, (b) un `<span className="truncate">` envolviendo el contenido (no `truncate` directo en el wrapper). Sin `min-w-0`, los items flex/grid no se contraen por defecto y el texto desborda en vez de cortarse con `...`. Patrón aplicado a las celdas Nom/Format/DateTime del header y de las filas de `SonilabLibraryView`/`LibraryFileItem`.
...
- `docs/superpowers/specs/2026-04-07-reset-configuracio-frontend.md`: el feature Factory Reset preserva explícitamente los presets del usuario (tanto el backend `me.preferences.userStyles` como la caché local `snlbpro_user_styles_<userId>`). Cualquier cambio en la estructura de los presets que rompiera esa preservación debe revisarse contra el spec.
