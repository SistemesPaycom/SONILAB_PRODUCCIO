...
async folderNameExists(ownerId: string, name: string, parentId: string | null) {
  ...
  async createFolder(ownerId: string, name: string, parentId?: string, category?: 'files' | 'media') {
  ...
  async updateFolder(ownerId: string, id: string, patch: Partial<Folder>) {
    // Conflict check: si s'estan canviant nom o parentId (move/rename),
    // verificar que no hi ha una altra carpeta amb el mateix nom a la ubicació destí.
    const patchAny = patch as any;
    if (patchAny.name !== undefined || patchAny.parentId !== undefined) {
      const current = await this.folderModel.findOne({ _id: id }).lean();
      if (current) {
        const effectiveName: string = patchAny.name ?? (current as any).name;
        const effectiveParentId: string | null =
          patchAny.parentId !== undefined
            ? (patchAny.parentId ?? null)
            : ((current as any).parentId ?? null);

        const conflict = await this.folderModel
          .findOne({
            _id: { $ne: id },
            ownerId,
            isDeleted: false,
        ...
        if (conflict) {
          throw new BadRequestException(
            `Ja existeix una carpeta amb el nom "${effectiveName}" en aquesta ubicació.`,
          );
        }
      }
    }
  ...
  async updateDocument(_ownerId: string, id: string, patch: Partial<Document>) {
    // Validació i conflict check quan s'estan canviant nom o parentId.
    const patchAny = patch as any;
    if (patchAny.name !== undefined || patchAny.parentId !== undefined) {
      const current = await this.docModel.findOne({ _id: id }).lean();
      if (current) {
        // Protecció d'extensió: no es pot canviar l'extensió d'un fitxer per error.
        if (patchAny.name !== undefined) {
          const getExt = (n: string) => { const i = n.lastIndexOf('.'); return i > 0 ? n.substring(i).toLowerCase() : ''; };
          const currentExt = getExt((current as any).name || '');
          const newExt = getExt(patchAny.name);
          if (currentExt && newExt !== currentExt) {
            throw new BadRequestException(
              `No es pot canviar l'extensió del fitxer (de "${currentExt}" a "${newExt || 'cap'}").`,
            );
          }
        }
        ...
        const conflict = await this.docModel
          .findOne({
            _id: { $ne: id },
            isDeleted: false,
            parentId: effectiveParentId,
        ...
        if (conflict) {
          throw new BadRequestException(
            `Ja existeix un fitxer amb el nom "${effectiveName}" en aquesta ubicació.`,
          );
        }
      }
    }
