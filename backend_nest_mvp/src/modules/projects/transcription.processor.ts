...
export class TranscriptionProcessor {
  ...
  async handle(job: BullJob<TranscribeJobPayload>) {
    ...
    try {
      ...
      // Subtitle gap/duration settings
      // Nota: MIN_SEG_DURATION_MS al frontend (constants.ts) ha de coincidir amb el default de minSubDurMs.
      const minSubGapMs = s.minSubGapMs != null ? Number(s.minSubGapMs) : 160;
      const enforceMinSubGap = s.enforceMinSubGap !== false;
      const minSubDurMs = s.minSubDurMs != null ? Number(s.minSubDurMs) : 100;
      ...
      if (usePurfviewExe) {
        ...
        if (engine === 'purfview-xxl') {
          ...
          args.push('--min-dur-ms', String(minSubDurMs));
        }
