...
export class ProjectsService {
  ...
  const folder = await this.library.createFolder(ownerId, name, dto.parentFolderId ?? undefined);
  ...
  async createProject(
    ...
    const exists = await this.library.folderNameExists(ownerId, name, resolvedParentId);
if (exists) {
  ...
  throw new (require('@nestjs/common').ConflictException)(`Project "${name}" already exists`);
    ...
    const folder = await this.library.createFolder(ownerId, name, resolvedParentId ?? undefined);
